...
(* direct_enable = 1 *) input   clk_en,
    ...
    output         ready,
    // Save-state snapshot ports (combinatorial out, registered in)
    output [63:0]  ss_state_out,   // {clk_div, regn, ctrl3, vol3, vol2, vol1, vol0, tone2[9:0], tone1[9:0], tone0[9:0]} padded to 64b
    input  [63:0]  ss_state_in,
    input          ss_state_load   // single-cycle strobe: restore all registers from ss_state_in
);
...
// State snapshot output: pack all config registers into 64 bits
// Bit layout: [63:60]=clk_div [59:57]=regn [56:54]=ctrl3
//             [53:50]=vol3 [49:46]=vol2 [45:42]=vol1 [41:38]=vol0
//             [37:28]=tone2 [27:18]=tone1 [17:8]=tone0  [7:0]=padding
assign ss_state_out = { clk_div, regn, ctrl3,
                        vol3, vol2, vol1, vol0,
                        tone2, tone1, tone0,
                        8'h00 };
...
always @(posedge clk)
    if( rst ) begin
        ...
        regn    <= ss_state_in[59:57];
        ctrl3   <= ss_state_in[56:54];
        vol3    <= ss_state_in[53:50];
        vol2    <= ss_state_in[49:46];
        vol1    <= ss_state_in[45:42];
        vol0    <= ss_state_in[41:38];
        tone2   <= ss_state_in[37:28];
        tone1   <= ss_state_in[27:18];
        tone0   <= ss_state_in[17:8];
    end
    else begin
        last_wr <= wr_n;
